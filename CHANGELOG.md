...
이 파일은 `post-commit` hook에 의해 자동 업데이트됩니다.

<!-- AUTO-GENERATED: CHANGELOG_START -->
## [Unreleased]

### Added
- 리뷰 생성 시 8가지 페르소나(10대~40대, 남/여) 랜덤 적용 — 매번 다른 사람이 쓴 것처럼 톤앤매너 변화
- 글자수 5단계 랜덤(100~400자) + ±15자 지터 적용으로 리뷰 길이 다양화
- 프롬프트에 `{{PERSONA}}` 플레이스홀더 추가
- 샘플 리뷰 10개 (`review/prompts/sample-reviews.md`) 페르소나별 작성
- 프롬프트에 지역 키워드(`{{REGION_LIST}}`), 음식 키워드(`{{FOOD_LIST}}`) 삽입 규칙 추가
- 프롬프트에 시점 표현 금지, 주차 언급 금지 조건 추가
- UI: 이용 순서 안내를 한 줄 인라인 형태로 변경
- UI: 리뷰 복사 / 네이버 리뷰 쓰기 버튼을 가로 2열 배치
- UI: 음식 사진·영상 안내 문구 추가
- CSS: 극소형 기기(320px) 대응 미디어쿼리 추가
- `update-changelog.sh`: KST 타임존 및 시:분 표시 적용

### Changed
- 프롬프트 톤 지시를 "20~40대 캐주얼" 고정에서 페르소나 기반 동적 톤으로 변경
- 글자수 범위를 200~400자 고정에서 5단계 랜덤으로 변경

## 2026/03/25

- [`91b7f2f`] 2026/03/25 11:41 UI 개선: 리뷰 텍스트 줄바꿈 수정, 이용 순서 안내 카드 위로 이동, 이모지/마크다운 제거
- [`1754f3e`] 2026/03/25 11:33 문서: CHANGELOG.md 자동 업데이트
